// 会話シーンの背景（横から見た一枚絵）。人物は顔のない黒い影で置く。
...
RPG.Scenes = (function () {
  ...
  PAINT.house = function (img) {
    ...
    var f = new Fig(img);
    f.person(446, 196, 130, { hair: "long", dir: -1 });
    f.ink([10, 8, 10], [200, 140, 100], 1, 0);
  };
  ...
  PAINT.plaza = function (img) {
    ...
    var f = new Fig(img);
    // 壇上のカガリ（長衣、木札の箱を掲げる）
    f.person(240, 166, 76, { robe: true, armsUp: true });
    f.ink([8, 6, 8], [240, 150, 70], 0, 1);
    img.rect(228, 104, 24, 12, function (x, y) { return rp(WOOD, 0.3 + (y === 104 ? 0.4 : 0), x, y); });
    // 手前で見守る人々（背を向けた頭と肩の列）
    for (var i = 0; i < 16; i++) {
      var px = 10 + i * 30 + (hash2(i, 0, 21) - 0.5) * 14, hh = 96 + hash2(i, 1, 22) * 26;
      f.person(px, H + 24, hh, { back: true, hair: hash2(i, 2, 23) < 0.4 ? "long" : null, robe: hash2(i, 3, 24) < 0.3 });
    }
    f.ink([6, 5, 6], [120, 70, 40], 0, -1);
  };
  ...
  PAINT.chief = function (img) {
    ...
    var f = new Fig(img);
    f.person(298, 210, 120, { sit: true, hunch: true, dir: -1, hair: "bald" });
    f.ink([8, 6, 6], [230, 130, 60], -1, 0);
  };
  ...
  PAINT.gate = function (img, variant) {
    ...
    if (variant === "closed") {
      // 門の上から見下ろす集落長トキ
      f.person(240, 54, 40, { robe: true });
      f.ink([8, 6, 8], [190, 130, 100], 1, 0);
    } else {
      f.person(240, 236, 70, { back: true, hair: "spiky", walk: true });
      f.ink([8, 6, 8], [210, 150, 110], 0, -1);
    }
  };
  ...
  PAINT.narrow = function (img) {
    ...
    var f = new Fig(img);
    f.birdPerson(242, 184, 140, -1);
    f.ink([10, 4, 4], [200, 50, 40], 0, -1);
    // 赤い羽を感じさせる縁の二重線
    img.glow(242, 110, 60, [180, 40, 30], 0.12);
  };
  ...
  PAINT.shrine = function (img) {
    ...
    var f = new Fig(img);
    f.person(240, 144, 62, { robe: true, armsUp: true });
    f.ink([4, 6, 8], [90, 200, 200], 0, -1);
    for (var i = 0; i < 5; i++) f.person(150 + i * 45 + (i > 1 ? 10 : 0), 262, 46, { robe: true, back: true });
    f.ink([4, 5, 7], [60, 130, 140], 0, -1);
  };
  ...
  return { canvasFor: canvasFor, ids: Object.keys(PAINT) };
})();
